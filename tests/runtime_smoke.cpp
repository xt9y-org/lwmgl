...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(64, 64, "lwmgl-runtime-cpp", nullptr, nullptr);
    ...
    std::uint32_t initial[256];
    for (std::uint32_t i = 0u; i < 256u; ++i) initial[i] = i;
    ...
    LWMGLFunction function = library ? Metal.createFunction(library, "add_one") : nullptr;
    LWMGLComputePipeline pipeline = function ? Metal.createComputePipeline(function) : nullptr;
    ...
    auto *values = static_cast<std::uint32_t *>(Metal.bufferContents(buffer));
    if (!values) return 8;
    for (std::uint32_t i = 0u; i < 256u; ++i) if (values[i] != i + 1u) return 9;
